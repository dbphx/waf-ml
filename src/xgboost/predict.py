...
class HTTPAttackPredictor:
    def __init__(self, model_dir=f"{PROJECT_ROOT}/models/xgboost", use_onnx=False):
    ...
    def _predict_probability(self, df):
        ...
        if self.use_onnx:
            outputs = self.sess.run(None, {self.input_name: self._onnx_input(X)})
    ...
    def predict_components(self, http_data):
        ...
        if component_predictions:
            ...
            decisive = self._predict_component(
                "path",
                "/",
                method=request.get("method", ""),
                ua=request.get("user_agent", ""),
...
if __name__ == "__main__":
    ...
    print(predictor.predict("id=1' OR '1'='1"))
